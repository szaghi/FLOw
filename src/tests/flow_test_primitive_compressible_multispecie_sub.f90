...
velocity = -1._R_P
primitive3 = primitive_compressible_multispecie(density=-0.875_R_P, &
                                                velocity=velocity,  &
                                                pressure=0._R_P,    &
                                                partial_densities=[-0.4375_R_P, -0.4375_R_P])
...
print "(A,2(F7.4,1X))", 'densities => 0.0625 - 0.5 = ', primitive4%partial_densities(1), primitive4%partial_densities(2)
...
print "(A,2(F7.4,1X))", 'densities => - (0.0625) = ', primitive3%partial_densities

print "(A,L1)", new_line('a')//'Are all tests passed? ', all(test_passed)
endprogram flow_test_primitive_compressible_multispecie_sub
